...
**Источник:** Итерация 14, ручное тестирование в Kubernetes (01.2026) - см. `testing/ITERATION14_TEST_RESULTS.md`

- **Доработка поиска: фильтрация нерелевантных запросов**  
  На запросы вне контекста базы знаний бот возвращает выдачу вместо ответа «ничего не найдено». Нужно усилить фильтрацию (пороги, эвристики или отдельная проверка «вне контекста»), чтобы на произвольный/нерелевантный текст не показывать результаты.  
  **Файлы:** `src/search.py` (пороги, `filter_out_of_context_results`, возможно расширение списка стоп-фраз).
...
8. **Автоматизированное тестирование форматирования и разбиения сообщений** (на будущее)
   - **Цель:** автоматические тесты для проверки форматирования и разбиения ответов поиска
   - **Файл:** Создать `testing/test_formatting.py` или расширить существующие тесты
   - **Покрытие:** TC-2.1.2, TC-2.1.3, TC-2.1.4, TC-2.3.1, TC-2.3.2
   - **Проверки:** генерация snippet для больших разделов (>500 символов), полный текст для маленьких (<500), удаление лишних пустых строк, разбиение сообщений >4096 символов, сохранение HTML при разбиении
   - **Приоритет:** низкий (после стабилизации поиска)
