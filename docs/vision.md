...
- **Данные**: в каталоге `data/` хранится кэш содержимого документа (опционально).
...
- **База знаний**: документ из Google Docs импортируется и хранится в формате Markdown в `data/`. Один Google Doc → один Markdown-файл. Структура документа (заголовки, разделы) сохраняется. Изображения из документа хранятся как ссылки на оригиналы в Google Docs.

- **Кэш документа** (опционально): JSON-файл в `data/` с полями:
  ...
  - `images`: массив ссылок на изображения из документа (опционально)
...
- **Импорт документа**: получение содержимого через HTTP-запрос к прямой ссылке документа (экспорт в текстовый формат) или через Google Docs API с использованием сервисного аккаунта. Документ импортируется в формат Markdown с сохранением структуры (заголовки, разделы) и сохраняется в `data/`.
- **Обработка изображений**: при импорте документа извлекаются ссылки на изображения из Google Docs. Изображения хранятся как ссылки на оригиналы в Google. При необходимости поддерживается загрузка превью определенного размера. В ответах пользователям изображения отправляются отдельными сообщениями после текстового ответа.
...
- **Хранение**: документ хранится в формате Markdown в `data/`. Дополнительно поддерживается JSON-кэш для быстрого доступа при поиске. Ссылки на изображения сохраняются в кэше.
...
- **Обратный индекс**: для быстрого поиска по ключевым словам используется обратный индекс, построенный при импорте документа. Индекс содержит соответствие ключевых слов разделам документа.
