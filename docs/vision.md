...
- **Заголовки разделов**: разделы документа определяются автоматически по форматированию Markdown (заголовки уровня 2 и выше, например `## 7. Название раздела`). Разделение происходит при парсинге документа, без необходимости в отдельном файле с заголовками.
...
Загрузка контента выполняется администратором вручную и не зависит от загрузки модели или векторизации.
...
- **Google Docs как источник**: документ является публичным и доступен по прямой ссылке. Для чтения аутентификация не требуется. Получение содержимого происходит через HTTP-запрос к прямой ссылке документа (экспорт в HTML-формат в ZIP-архиве с изображениями) или через Google Docs API с использованием сервисного аккаунта.
- **Импорт документа**: выполняется администратором вручную через команду `/admin import_content` или отдельный скрипт. Документ импортируется в формате HTML и сохраняется в `data/knowledge.html`. Изображения извлекаются из ZIP-архива и сохраняются в папке `data/images/`. Заголовки разделов определяются автоматически при парсинге Markdown-документа по форматированию заголовков (например, `## 7. Название раздела`).
...
- **Хранение**: документ хранится в формате HTML в `data/knowledge.html`. Изображения хранятся локально в папке `data/images/`. Метаданные (пути к изображениям, дата обновления, document_id) сохраняются в JSON-кэше (`data/knowledge_cache.json`).
...
- **Векторизация разделов**: выполняется администратором вручную через команду `/admin vectorize` или отдельный скрипт. Процесс векторизации:
  1. Загружает заголовки разделов из `data/sections.json`
  2. Разбивает документ из `data/knowledge.html` на разделы по заголовкам
...
- **Команды**:
  ...
  - Админские команды (только для пользователей из белого списка админов):
    - `/admin import_content` — импорт контента из Google Docs (сохранение HTML и изображений в локальные файлы)
