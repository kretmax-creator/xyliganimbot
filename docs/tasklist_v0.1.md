...
**Задачи**:
- [x] Создать структуру для автоматического извлечения заголовков разделов из Markdown
...
- [x] Добавить проверки наличия необходимых данных при обработке запросов:
  ...
  - наличие Markdown-файла `data/knowledge.md`
...
**Задачи**:
- [ ] **Реализовать адаптивный порог min_score** для фильтрации нерелевантных запросов
...
- [ ] **Добавить абсолютный порог для первого результата**
...
**Задачи**:
- [ ] **Реализовать обработку отрицаний в запросах**
  - Расширить функцию `preprocess_query()` в `src/search.py`
  - Добавить функцию `preprocess_negation_query()` для распознавания отрицаний:
    - Паттерны: "но не", "исключая", "без", "кроме"
  - Добавить функцию `filter_excluded_sections()` для фильтрации результатов с исключаемыми терминами
  - Интегрировать в функцию `search()` перед вызовом `semantic_search()`
  - Протестировать на TC-1.2.2
- [ ] **Добавить boost для точных совпадений ключевых слов**
  - Создать функцию `boost_exact_matches()` в `src/search.py`
  - Повышать score разделов с точными совпадениями ключевых слов из запроса
  - Заголовок раздела имеет больший вес (x2) чем текст
  - Применять boost после `semantic_search()`, перед фильтрацией
  - Пересортировывать результаты после boost
  - Протестировать на TC-1.8.4
- [ ] **Улучшить фильтрацию нерелевантных запросов вне контекста**
  - Создать функцию `filter_out_of_context_results()` в `src/search.py`
  - Если максимальный score < 0.7, возвращать пустой список
  - Интегрировать в функцию `search()` после `semantic_search()`
  - Протестировать на TC-1.9.4
...
- [ ] **Повторное тестирование**
  - Повторить ручное тестирование всех проблемных тесткейсов из Фазы 1 и Фазы 2
  - Зафиксировать результаты в `docs/MANUAL_TEST_RESULTS_E5.md`
  - Проверить достижение целевых метрик: >90% прохождения тестов
- [ ] **Проверка метрик качества**
  - Средний score релевантных результатов должен быть > 0.85
  - Средний score нерелевантных результатов должен быть < 0.5 (или пустой список)
  - Все критические тесткейсы должны проходить (PASS)

**Результат Фазы 3**: Подтверждено улучшение качества поиска. Достигнуты целевые метрики.
...
- [ ] **Улучшить обработку ошибок** при отсутствии модели и embeddings
  - Улучшить проверки в `src/handlers/messages.py` (функция `handle_search_query()`)
  - Добавить понятные сообщения пользователю при отсутствии модели/embeddings
  - Проверить корректность fallback на token-based поиск при отсутствии embeddings
...
**Результат**: Бот устойчив к ошибкам, пользователи получают понятные сообщения. Улучшена обработка ошибок при отсутствии модели и embeddings.
