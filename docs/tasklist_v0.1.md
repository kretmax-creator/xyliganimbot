...
**Цель**: Создать манифесты для развертывания в Kubernetes и проверить развертывание.
...
- [x] **Ручное тестирование развертывания в кластере**
  - Подтверждена работоспособность в кластере на VirtualBox с containerd.
  - Исправлены ошибки binding томов (добавлены `labels` и `matchLabels` в PV/PVC).
  - Скорректирован `k8s/configmap.yaml` с реальными ID чата и админа.

**Результат**: Готовые и протестированные манифесты. Бот успешно запускается в Kubernetes, подключается к Telegram и обрабатывает сообщения.
...
**Чек-лист и отчёт**: `testing/ITERATION14_TEST_CHECKLIST.md`, `testing/ITERATION14_TEST_RESULTS.md`.

**Задачи**:
- [x] Ручное тестирование всех команд (по чек-листу)
- [x] Тестирование поиска с различными запросами
- [x] Тестирование импорта документа (скрипт вне бота) — не проверялось, не требуется в текущем сценарии
- [x] Тестирование обработки ошибок
- [x] Тестирование белых списков
- [x] Тестирование админских команд
- [x] Исправление найденных багов — текст /help обновлён (вариант A); доработка фильтрации нерелевантных запросов вынесена в бэклог
- [x] Проверка логирования и аудита

**Результат**: Протестированный бот в Kubernetes. 11 пунктов чек-листа пройдено, 1 (фильтрация нерелевантных запросов) — в бэклог. Текст команды /help уточнён.
...
**Задачи**:
- [x] Обновить `README.md` с полными инструкциями (ссылки, конфиг, команды, SSH)
- [x] Создать документацию по командам бота (`docs/commands.md`)
- [x] Добавить примеры конфигурации (в README, ссылка на config.yaml.example)
- [x] Создать инструкции по развертыванию через SSH (`docs/deploy_ssh.md`)
- [x] Составить диаграммы (Mermaid): architecture, command_routing, deployment, network; ревью существующих (data_flow_*)
- [x] Проверить все ссылки и примеры в документации (исправлена ссылка на TESTING.md)
- [x] Создать `CHANGELOG.md`
- [x] Финальная проверка кода и документации

**Результат**: Полная документация, готовый к релизу MVP. Диаграммы проверены (исправлены лексические ошибки Mermaid в architecture.md и network.md).

---

### Итерация 16: Улучшение качества поиска и ранжирования
**День 16**

**Цель**: Улучшить качество поиска за счёт расширения словаря нормализации, обработки опечаток и ранжирования точных фактов.

**Задачи**:
- [ ] **Расширить словарь нормализации** (рекомендация: вариант A)
  - Добавить в `QUERY_REPLACEMENTS` в `src/search.py`: "удаленка" → "vrm", "учетка" → "учетная запись", при необходимости "пин" → "pin"
  - Без выноса в конфиг
- [ ] **Добавить обработку опечаток** (рекомендация: вариант C)
  - Добавить `pyspellchecker` в `requirements.txt`
  - В `preprocess_query()` после замен по словарю: проверять опечатки только у слов, которых нет в `QUERY_REPLACEMENTS` и в списке доменных терминов
  - Ввести словарь доменных терминов (VPN, ВРМ, vrm, innotech, rutoken, sakura и т.д.) в `src/search.py`, чтобы их не исправлять
- [ ] **Улучшить ранжирование** для точных фактов — адреса, пароли, телефоны (рекомендация: вариант A)
  - В `semantic_search()` после получения `hits`: для каждого результата проверять наличие в запросе и в тексте раздела паттернов (URL, email, телефон, последовательности цифр)
  - При совпадении паттерна — применять boost к score (например `score *= 1.2` или `score += 0.1`, ограничить до 1.0), пересортировать результаты
  - Подобрать порог и величину boost по тесткейсам TC-1.1.3, TC-1.8.1, TC-1.8.2, TC-1.8.3

**Результат**: Улучшено качество поиска за счёт расширенной нормализации, обработки опечаток и ранжирования точных фактов.
