...
**Задачи**:
- [ ] Создать модуль `src/search.py` с векторизацией разделов
...
- [ ] Добавить зависимости: `sentence-transformers`, `torch` в `requirements.txt`
- [ ] Создать папку `models/` для локального хранения моделей
- [ ] Реализовать функцию `load_embedding_model()` для загрузки модели из `models/`
- [ ] Реализовать функцию `vectorize_sections()` для векторизации разделов через embedding-модель
- [ ] Обновить `build_index_from_html()` → `build_embeddings_from_html()` для векторизации
- [ ] Обновить `save_index_to_cache()` для сохранения векторов (опционально, можно хранить в памяти)
- [ ] Обновить `load_index_from_cache()` для загрузки векторов
- [ ] Интегрировать векторизацию в процесс импорта документа (`google_docs.py`)
- [ ] Добавить обработку ошибок при отсутствии модели или файла с заголовками

**Результат**: Разделы документа векторизуются при импорте через embedding-модель (`paraphrase-multilingual-MiniLM-L12-v2`). Векторы хранятся в памяти (или опционально в кэше). Реализованы функции: `load_embedding_model()`, `vectorize_sections()`, `build_embeddings_from_html()`. Модель загружается из локальной папки `models/` для автономной работы. Векторизация выполняется автоматически при импорте документа.
