...
- [ ] Создать структуру для связи изображений с разделами документа (опционально, отложено для MVP)

**Результат**: Создана структура для хранения метаданных документа и заголовков разделов. Заголовки разделов управляются вручную через JSON-файл. Изображения хранятся локально. Реализованы функции `load_cache()`, `save_cache()`, `check_cache_needs_update()`, `get_image_paths()` в модуле `src/google_docs.py`. При импорте документа автоматически создается `data/knowledge_cache.json` с метаданными.
...
**Задачи**:
- [x] Создать модуль `src/search.py`
...
- [x] Реализовать построение двухуровневого обратного индекса:
...
- [x] Сохранять индекс в JSON-кэш
- [x] Реализовать обновление индекса при импорте документа
- [x] Добавить базовую нормализацию текста (приведение к нижнему регистру, удаление знаков препинания)
- [x] Добавить обработку ошибок при отсутствии файла с заголовками

**Результат**: Двухуровневый обратный индекс строится при импорте документа. Разделение на разделы происходит по заголовкам из JSON-файла, управляемого вручную. Индексация включает как разделы, так и содержимое разделов. Реализованы функции: `load_sections()`, `normalize_text()`, `tokenize_text()`, `parse_html_sections()`, `build_search_index()`, `build_index_from_html()`. Индекс автоматически сохраняется в `knowledge_cache.json` при импорте документа.
...
**Задачи**:
- [x] Реализовать функцию поиска по обратному индексу
- [x] Реализовать ранжирование результатов по количеству совпадений
- [x] Ограничить количество результатов (5 наиболее релевантных)
- [x] Реализовать извлечение текста найденных разделов
- [x] Добавить обработку случая "ничего не найдено"
- [x] Логирование результатов поиска

**Результат**: Поиск работает, возвращает ранжированные результаты. Реализованы функции `search()` и `get_section_text()` в модуле `src/search.py`. Поиск использует двухуровневый индекс, ранжирует результаты по релевантности (совпадения в заголовках имеют больший вес), ограничивает результаты до 5 наиболее релевантных разделов. Добавлена оптимизация - возможность передавать уже распарсенные разделы для избежания повторного парсинга HTML.
